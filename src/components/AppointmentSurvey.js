...
export default function AppointmentSurvey() {
  ...
  useEffect(() => {
    ...
    // Removed the scroll trigger animation for the component itself since it's now on a dedicated page usually
    // But kept purely for entrance effect if needed. 
    // If it's on the contact page, we might want it to just appear. 
    // Let's keep a simple fade in.
  ...
  return (
    <div ref={sectionRef} className="w-full flex flex-col items-center justify-center">
      <div className="survey-container w-full bg-white/80 backdrop-blur-md p-8 md:p-12 rounded-3xl shadow-2xl border border-white/50 relative overflow-hidden">
